...
import org.elasticsearch.river.jolokia.support.HealthMonitorThread;
 ...
 * <p/>
 * Bulk indexing is implemented concurrently. Therefore, many rivers can pass their
...
public class SimpleRiverMouth implements RiverMouth {
    ...
    private static final ThreadFactory THREAD_FACTORY_HEALTH = EsExecutors.daemonThreadFactory("jolokia-river-health");
    ...
    @Override
    public SimpleRiverMouth indexTemplate(String indexTemplate) {
    ...
    @Override
    public String index() {
        return getRealIndex(indexTemplate);
    }
    ...
    public String getRealIndex(String index) {
		Pattern p = Pattern.compile("(\\[(.*)\\])");
		Matcher m = p.matcher(index);
		if (m.find()) {
			String datePattern = m.group(2);
			SimpleDateFormat fmt = new SimpleDateFormat(datePattern);		
			String realIndex = m.replaceFirst(fmt.format(new Date()));		
			return realIndex;
		} else {
			return index;
		}
